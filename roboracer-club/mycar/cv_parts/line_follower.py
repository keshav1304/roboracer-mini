...
class LineFollower:
    ...
    It does a histogram to find the pixel of maximum yellow. Then it uses that iPxel
    ...
    def _steer_full_scale(self):
        """Expected |P-term| when the line is at the farthest image edge from target."""
        target = self.target_pixel if self.target_pixel is not None else self.image_w // 2
        max_err = max(target, self.image_w - 1 - target)
        return max(abs(self.pid_st.Kp) * max_err, 1e-6)

    def _normalize_steering(self, raw):
        """
        Map PID output to [-1, 1] proportionally: scale by full-scale error,
        then tanh so large commands saturate smoothly instead of hard clipping.
        """
    ...
    def get_i_color(self, cam_img):
        ...
        # red / orange / yellow: S,V 80-255; hue wraps (red at 0 and 170-179)
    ...
    def run(self, cam_img):
        ...
        if confidence >= self.confidence_threshold:
            ...
            raw_steering = self.pid_st(max_yellow)
