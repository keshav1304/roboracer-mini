...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if camera_type == "stereo":
        ...
        V.add(cam, inputs=[],
              outputs=['cam/raw_image_array', 'cam/depth_array',
    ...
    #
    # Rotate / undistort camera frames before CV, recording, and web UI
    #
    from image_preprocess import ImagePreprocessor
    V.add(ImagePreprocessor(cfg),
          inputs=['cam/raw_image_array'],
          outputs=['cam/image_array'])

    use_cv_autopilot = getattr(cfg, 'USE_CV_AUTOPILOT', False) and not model_path
    web_image = 'ui/image_array' if use_cv_autopilot else 'cam/image_array'
    ...
    V.add(ctr,
        inputs=[web_image, 'tub/num_records'],
    ...
    #
    # Computer vision autopilot (when USE_CV_AUTOPILOT and no --model)
    #
    if use_cv_autopilot:
        ...
        cv_pid = PID(
            Kp=getattr(cfg, 'CV_PID_P', cfg.PID_P),
            Ki=getattr(cfg, 'CV_PID_I', 0),
            Kd=getattr(cfg, 'CV_PID_D', cfg.PID_D),
        )

        def dec_cv_pid_d():
            cv_pid.Kd -= cfg.CV_PID_D_DELTA
            logger.info("cv pid: d- %f", cv_pid.Kd)
        ...
        def dec_cv_pid_p():
            cv_pid.Kp -= cfg.CV_PID_P_DELTA
            logger.info("cv pid: p- %f", cv_pid.Kp)
        ...
        if isinstance(ctr, JoystickController):
            ...
            if cfg.DEC_PID_P_BTN and cfg.CV_PID_P_DELTA:
                ctr.set_button_down_trigger(cfg.DEC_PID_P_BTN, dec_cv_pid_p)
            if cfg.INC_PID_P_BTN and cfg.CV_PID_P_DELTA:
                ctr.set_button_down_trigger(cfg.INC_PID_P_BTN, inc_cv_pid_p)
            if cfg.DEC_PID_D_BTN and cfg.CV_PID_D_DELTA:
                ctr.set_button_down_trigger(cfg.DEC_PID_D_BTN, dec_cv_pid_d)
            if cfg.INC_PID_D_BTN and cfg.CV_PID_D_DELTA:
                ctr.set_button_down_trigger(cfg.INC_PID_D_BTN, inc_cv_pid_d)
    ...
    if cfg.RECORD_DURING_AI and not use_cv_autopilot:
...
class UserPilotCondition:
    """Select user vs autopilot run flags and which image feeds the web UI."""
    ...
    def run(self, mode, user_image, pilot_image):
        ...
        if self.show_pilot_image and pilot_image is not None:
            return False, True, pilot_image
        return False, True, user_image


class ToggleRecording:
    def __init__(self, auto_record_on_throttle, record_during_ai):
        self.record_during_ai = record_during_ai
        self._toggle = False
    ...
    def run(self, mode, recording):
        if self._toggle:
            ...
            self._toggle = False
        if mode != 'user' and self.record_during_ai:
            return True
        return recording
...
def add_cv_controller(
        V, cfg, pid,
        module_name="cv_parts.line_follower",
        class_name="LaneKeeper",
        inputs=['cam/image_array'],
        outputs=['pilot/angle', 'pilot/throttle', 'cv/image_array'],
        run_condition="run_pilot"):
